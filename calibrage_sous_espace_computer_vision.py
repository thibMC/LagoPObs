...
def spec(signal, taille_fenetre, overlap, taille_env, overlap_env, fs, taille_max, freqs_interet):
    ...
    # Pad à la taille max
    max_len = max([len(sTFT.t(int(taille_max*fs))),
                   len(sTFT_env.t(int(taille_max*fs)))
                   ])
...
spectrogrammes = []
for w in wlen:
    s = [spec(signaux[k], w, ovlp, wlen_env, ovlp_env, freq_ech[k], max_dur, f_filt) for k in range(len(signaux))]
...
from sklearn.mixture import GaussianMixture
from sklearn.cluster import HDBSCAN, AgglomerativeClustering, KMeans, BisectingKMeans, MeanShift
...
def distance_matches(matcher, kp_des1, kp_des2, n_closest):
    ...
    if matches and len(matches) <= n_closest:
        ...
        dist = 1e6
    return(dist)
def cluster_spectros(liste_spectros, nom_males, color_labels,
                     detector_methode = "SIFT",
                     methode_linkage="ward", n_keys=10, plot=True):
    ...
    dist_images = [distance_matches(matcher,
                                   keypoints_descriptors[i],
                                   keypoints_descriptors[j], n_keys)
    ...
    if plot:
        ...
        dendrogram(linkage_data, labels=nom_males,
                   leaf_font_size=10)
...
for i,s in enumerate(spectrogrammes):
    for i_n,n in enumerate(n_features):
        a_rand[i,i_n] = cluster_spectros(s, indiv, int_ind, detector_methode = "BRISK", n_keys=n, plot=False)
t2 = time.time()
print(t2-t1)
...
for i, sp in enumerate(spectrogrammes):
    ...
    qualite_UMAP = np.array([eval_UMAP(s_flat, int_ind, int(p[0]), p[1], int(p[2])) for p in parametres])
...
param_select = parametres[param_best[1]].astype(int) # => voisins : 12/distance : 0/nombre composantes : 52
spectro_best = spectrogrammes[param_best[0]]
spectro_best_flat = np.array([s.T.ravel() for s in spectro_best])
model_UMAP_CC = umap.UMAP(n_components=param_select[2], min_dist=param_select[1], n_neighbors=param_select[0], random_state=graine).fit(spectro_best_flat, y=int_ind)
...
# Transformation en spectros
t1 = time.time()
spectro_alpyr = [spec(alpyr[k], wlen[param_best[0]], ovlp, wlen_env, ovlp_env, alpyr_fs[k], max_dur, f_filt)
...
plt.scatter(sous_espace_alpyr[alpyr_pop=="A",0], sous_espace_alpyr[alpyr_pop=="A",1], c=alpyr_int_ind[alpyr_pop=="A"])
plt.subplot(224)
plt.suptitle("Répartition des individus des Pyrénées")
plt.scatter(sous_espace_alpyr[alpyr_pop=="P",0], sous_espace_alpyr[alpyr_pop=="P",1], c=alpyr_int_ind[alpyr_pop=="P"])
plt.show()
...
print("Nombre individus Pyrénées : " + str(len(np.unique(alpyr_indiv[alpyr_pop=="P"]))))
